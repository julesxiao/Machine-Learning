...
# =============================================================================
# fill in code here
    indexList = np.nonzero(Ynew==1)
    index = indexList[1]
    posX = Xnew[:,[i for i in range(np.shape(Xnew)[1]) if i in index]]
    indexList = np.nonzero(Ynew==-1)
    index = indexList[1]
    negX =  Xnew[:,[i for i in range(np.shape(Xnew)[1]) if i in index]]

    num_pos_feature1 = np.sum(posX,axis=1)
    num_neg_feature1 = np.sum(negX,axis=1)

    Numposp = sum(np.sum(posX,axis=1))
    Numnegp = sum(np.sum(negX,axis=1))
   
    posprob =num_pos_feature1/Numposp
    negprob = num_neg_feature1/Numnegp
